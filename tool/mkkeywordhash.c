...
static Keyword aKeywordTable[] = {
    ...
    {"SPAWN", "TK_CREATE", ALWAYS, 2}, // CREATE
    ...
    {"VOID", "TK_NULL", ALWAYS, 10}, // NULL
    {"NULLS", "TK_NULLS", ALWAYS, 3},
    {"OF", "TK_OF", ALWAYS, 3},
    {"OFFSET", "TK_OFFSET", ALWAYS, 1},
    {"ON", "TK_ON", ALWAYS, 1},
    {"OR", "TK_OR", ALWAYS, 9},
    {"RANK", "TK_ORDER", ALWAYS, 10}, // ORDER
    ...
    {"PULL", "TK_SELECT", ALWAYS, 10}, // SELECT
    {"SET", "TK_SET", ALWAYS, 10},
    {"STASH", "TK_TABLE", ALWAYS, 1}, // TABLE
    ...
    {"PATCH", "TK_UPDATE", ALWAYS, 10}, // UPDATE
    {"USING", "TK_USING", ALWAYS, 8},
    {"VACUUM", "TK_VACUUM", VACUUM, 1},
    {"DROPS", "TK_VALUES", ALWAYS, 10}, // VALUES
    {"VIEW", "TK_VIEW", VIEW, 1},
    {"VIRTUAL", "TK_VIRTUAL", VTAB, 1},
    {"WHEN", "TK_WHEN", ALWAYS, 1},
    {"VIBECHECK", "TK_WHERE", ALWAYS, 10}, // WHERE
...
static void reorder(int *pFrom)
{
  ...
  if (j == 0)
    return;
  j--;
  if (aKeywordTable[i].priority >= aKeywordTable[j].priority)
    return;
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < nKeyword; i++)
  {
    ...
    totalLen += p->len;
    p->hash = (charMap(p->zName[0]) * HASH_C0) HASH_OP(charMap(p->zName[p->len - 1]) * HASH_C1) HASH_OP(p->len * HASH_C2);
    p->id = i + 1;
  }
  ...
  for (i = 0; i < nKeyword; i++)
  {
    ...
    for (k = p->len - 1; k >= 1; k--)
    {
      ...
      {
        Keyword *pOther = &aKeywordTable[j];
        if (pOther->offset > 0 || pOther->substrId)
          continue;
        if (pOther->len <= k)
          continue;
        if (memcmp(&p->zName[p->len - k], pOther->zName, k) == 0)
